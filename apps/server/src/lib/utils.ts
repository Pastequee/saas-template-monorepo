import { sql } from '@repo/db'
import { tryCatch } from '@repo/utils'
import { Elysia } from 'elysia'

import type { AppDeps } from '#deps'

import { createAuthMacro } from './auth.macros'

export const createUtils = (deps: AppDeps) => {
	const utilsLifecycles = new Elysia({ name: 'utils-lifecycles' })
		.onError(({ status, code }) => {
			if (deps.env.APP_ENV !== 'production') {
				return
			}

			// Obfuscate unpredictable errors
			if (
				code === 'INTERNAL_SERVER_ERROR' ||
				code === 'UNKNOWN' ||
				(typeof code === 'number' && code >= 500)
			) {
				return status(500, { message: 'Internal server error' })
			}

			// oxlint-disable-next-line no-useless-return
			return
		})
		.as('global')

	const utilsEndpoints = new Elysia({ name: 'utils', tags: ['Utils'] })
		.use(createAuthMacro(deps))
		.get('/', () => 'Application API' as const)
		.get('/health', async () => {
			const [, error] = await tryCatch(Promise.resolve(deps.db.execute(sql`SELECT 1`)))
			const dbStatus = error ? ('unhealthy' as const) : ('healthy' as const)

			return {
				commitHash: deps.env.COMMIT_HASH,
				database: dbStatus,
				environment: deps.env.NODE_ENV,
				status: 'healthy' as const,
				timestamp: new Date().toISOString(),
			}
		})

	return new Elysia().use(utilsLifecycles).use(utilsEndpoints)
}
