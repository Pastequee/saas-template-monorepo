...
export const createListingsRouter = (deps: AppDeps) => {
	const listingsService = ListingsService(deps)
	...
	const getImageUrl = (imageKey: string | undefined | null) => {
		...
		return deps.fileStorage.getUrl(imageKey)
	}

	return new Elysia({ name: 'listings', tags: ['Listing'] })
