...
export function useFormStepper<
  TSchemaArray extends readonly [ZodObject, ...ZodObject[]],
>(schemas: TSchemaArray, options?: UseFormStepperOptions) {
...
export type Steps<TStepCount extends number> = Record<
  LengthUnion<TStepCount>,
  React.ReactNode
>

type LengthUnion<N extends number> = Exclude<LengthUnionHelper<N>, 0>

type LengthUnionHelper<
  N extends number,
  A extends number[] = [],
> = A['length'] extends N
  ? A[number] | N
